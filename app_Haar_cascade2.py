...
def extract_face_features(face_image):
    """Extract features from the face image using LBP (Local Binary Patterns)."""
...
def compare_faces(face_features):
    """Compare the extracted features with known faces."""
...
class FaceRecognitionApp(QWidget):
    ...
    def upload_image(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(self, "Select Image", "", "Images (*.png *.jpg *.bmp)", options=options)
        ...
        if file_name:
            ...
            rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            resized_face = cv2.resize(rgb_img, (100, 100))  # Resize for comparison
            name = os.path.basename(file_name).split('.')[0]
            known_faces.append(resized_face)
            known_names.append(name)
            cv2.imwrite(os.path.join(KNOWN_FACES_FOLDER, f"{name}.jpg"), img)

            QMessageBox.information(self, "Upload Success", f"Image of {name} uploaded successfully!")

    def mark_attendance_with_action(self):
        """Mark attendance with entry/exit action."""
        name = self.name_input.text().strip()
        if name and name in known_names:
            action = self.action_combo.currentText()
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            data = pd.read_csv(attendance_file) if os.path.exists(attendance_file) else pd.DataFrame(columns=["Name", "Time", "Action"])
            data = data.append({"Name": name, "Time": current_time, "Action": action}, ignore_index=True)
            data.to_csv(attendance_file, index=False)
            self.update_label(f"{action} marked for {name} at {current_time}")
        else:
            self.update_label("Unknown or unregistered name.")

    def view_attendance_summary(self):
        """Display the attendance summary."""
        if os.path.exists(attendance_file):
            df = pd.read_csv(attendance_file)
            QMessageBox.information(self, "Attendance Summary", df.to_string(index=False))
        else:
            QMessageBox.warning(self, "No Attendance", "No attendance records found.")
